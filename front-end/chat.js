...
ws.onmessage = event => {
    ...
    // Format for display
    const room = msgObj.room || "General";
    const from = msgObj.from || "Unknown";
    const message = msgObj.message || "";
    const time = new Date(Number(msgObj.timestamp)).toLocaleTimeString();

    appendMessage(`[${room}] ${from} @ ${time}: ${message}`);
};
